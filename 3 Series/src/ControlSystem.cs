...
*/
                ...
                else
                {
                    ...
                    foreach (Level mic in config.mics)
                    {
                        ...
                        if (ui.location.Owner == config.locations[mic.room - 1].Owner)
                        {
                            ...
                            ui.Device.SmartObjects[SMART_ID_MICS].BooleanInput[String.Format("fb{0}", micIndex * 3)].BoolValue = mic.mute;
                        }
                    }
        ...
        public void JoinRoomsToggle(Location loc, Location loc2)
        {
            ...
            SetAudioPreset(RoomIsJoined(loc) ? 0 : 1);
            doJoinFeedback();
        }
        #endregion
                ...
                if (type == SwitchType.VIDEO)
                {
                    ...
                    if (source > 0)
                    {
                        ...
                        if (displays != null && displays[(byte)(destVidMatrix - 1)] != null && displays.Count >= destVidMatrix)
                        {
                            ...
                            // source select
                            if ((config.vidOutputs[destVidMatrix - 1].brand == "Panasonic" && !InitialParametersClass.ControllerPromptName.Contains("RMC3")) ||
                    ...
                    else // clear
                    {
                        ...
                        if (vidMatrix.Outputs != null && vidMatrix.Outputs.Count >= destVidMatrix)
                        {
                            if (vidMatrix.SwitchType.ToString().Contains("HdMd") && vidMatrix.Outputs[destVidMatrix].VideoOutFeedback != null)
                                vidMatrix.Outputs[destVidMatrix].VideoOut = vidMatrix.Outputs[destVidMatrix].VideoOutFeedback; // toggle off
                            else
        ...
        public void SetMicLevel(Level mic)
        {
            ...
            if (audDspComms != null)
            {
                ...
                //if (InitialParametersClass.ControllerPromptName.Contains("RMC3")) // single room
                if (config.locations.Find(x => x.Name.Contains("Theatre")) != null)
        ...
        public void SetMicmute(Level mic)
        {
            ...
            foreach (UiWithLocation ui in uis)
                ui.Device.SmartObjects[SMART_ID_MICS].BooleanInput[String.Format("fb{0}", index*3 + 3)].BoolValue = mic.mute;
        }
        ...
        {
            CrestronConsole.PrintLine("*** RecallConfig: {0}\\{1}",filePath, fileName);
            Config data;
                ...
                else
                {
                    for (int i = 0; i < config.vidInputs.Count; i++)
                    {
                        ...
                        else if (input.devType == (ushort)VidDev.DOCCAM)
                        {
                            ...
                            {
                                if (InitialParametersClass.ControllerPromptName.Contains("RMC3") &&
        ...
        private void configLights()
        {
            ...
            if (config.lights.address != null && config.lights.address.Length > 0)
            {
                    ...
                    config.lights.address)
                {
                    ...
                    uint port = (uint)(InitialParametersClass.ControllerPromptName.Contains("RMC3") || config.locations.Find(x => x.Name == "11.21") != null ? 1 : 2); // 4.11 COM-2
